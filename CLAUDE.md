...
JDK 25 toolchain is configured in `gradle.properties` (`jvmVersion = 25`, `kotlinVersion = 2.4.0`). Use the wrapper. (`chupacabra` v0.2.1+ is built for JVM 25, so the consumer toolchain must match.)
...
- `generate/text/` — annotated Bible text in multiple formats. `AnnotatedDoc` + `Annotation` are the abstraction layer over `StudyData` ranges (built by `BibleAnnotationPipeline`, walked by `BibleTextWalker` which dispatches format-agnostic events to a `BibleTextHandler`); format-specific writers live in `typst/`, `latex/`, `docx/`. **Typst is the actively-developed format; LaTeX and DOCX are effectively deprecated** — kept working for now, but new text features land in Typst only (other writers should reject an unsupported option via `BibleTextWriter.supports(...)` or the feature guard in `BibleTextPipeline.render`, which skips-with-message rather than aborting).
  - **Configuration model** (see `TextPreset.kt`): unified under `TextOptions` (combining all layout, styling, and content feature options). A `TextPreset` (`tbb`, `marks`, `plain`) bundles a default set of options alongside a description. Custom presets can be defined dynamically in properties files (CWD or global) via `preset.<name>.<option>`. `TextOverrides` carries tri-state per-field overrides (`null` = inherit) resolved via `TextPreset.resolve(...)` to produce the final `TextOptions`.
  - **`generateBibleTexts` has two modes**: with no `--preset`/override flags it emits the curated *pack* (plain/unique/full × tbb/marks × formats); naming a preset or setting any override switches to *single* mode (one resolved `TextOptions` → one document per format). CLI exposes `--preset` plus tri-state override flags (`--font-size`, `--[no-]two-columns`, `--[no-]inline-chapter-labels`, `--[no-]page-break-chapters`, `--[no-]underline-unique`, `--[no-]highlight`, `--[no-]verse-per-line`, `--[no-]chapter-end-lines`). Note `--inline-chapter-labels` is the inverse of the internal `useHeadingsForChapters`.
